...
export default function connectToAwp() {
    ...
    function connect() {
        ...
        adminRequest.get('?action=agentwp_generate_unique_verification_key').then((response) => {
            document.location=`${agentwp_settings.api_host}/connect_site?website=${encodeURIComponent(response.data.home_url)}&user_email=${agentwp_settings.user_email}&uid=${response.data.key}`;
        });
    }
    ...
    return (
        <div>
            ...
            <br/>
            {agentwp_settings.site_id && !loggedIn && <Button onClick={authorize} variant="primary" className="button" disabled={authorizing} isBusy={authorizing}>Login to AWP</Button>}
            {agentwp_settings.site_id && loggedIn && <Button onClick={logout} isDestructive className="button" >Log out</Button>}
        </div>
    )
}
