...
export function User({user}: {user: agentwpUser}) {
    ...
    function setManageAgentwpUsers() {
        ...
        adminRequest.post("agentwp_update_user", { user: theUser.id, manage_agentwp_users: !theUser.manage_agentwp_users });
    }

    function setAgentwpAccess() {
        setTheUser({...theUser, agentwp_access: !theUser.agentwp_access});
        adminRequest.post("user", { user: theUser.id, agentwp_access: !theUser.agentwp_access });
    }

    return (
      <div key={theUser.id} className="flex justify-start gap-4 items-center p-2 bg-gray-100 rounded-xl mt-4">
          ...
          <div className={"flex-grow flex justify-end"}>
              <input disabled={theUser.is_current_user} type="checkbox" checked={theUser.agentwp_access} onChange={() => setAgentwpAccess()} />
          </div>
      </div>
  );
}
