...
namespace WpAi\AgentWp\Modules\Vite;
...
window.__vite_plugin_react_preamble_installed__ = true;';
    ...
    public function load_production_asset(object $manifest, string $entry, array $options): ?array
    {
            ...
            $check_imports = function (array $imports) use (&$check_imports, &$assets, $manifest, $url, $options): void {
                foreach ($imports as $import) {
                    ...
                    if (! empty($import_item->css)) {
                        $this->register_stylesheets($assets, $import_item->css, $url, $options);
                    }
                }
            };
        ...
        if (! empty($item->css)) {
            $this->register_stylesheets($assets, $item->css, $url, $options);
        }
     ...
     */
    public function register_stylesheets(array &$assets, array $stylesheets, string $url, array $options): void
    {
     ...
     * @param  array  $options  Enqueue options.
     */
    public function register_asset(string $manifest_dir, string $entry, array $options): ?array
    {
        try {
     ...
     * @param  array  $options  Enqueue options.
     */
    public function enqueue_asset(string $manifest_dir, string $entry, array $options): bool
    {
        $assets = $this->register_asset($manifest_dir, $entry, $options);
