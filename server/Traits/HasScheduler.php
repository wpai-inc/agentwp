...
trait HasScheduler
{
    ...
    protected function validateActionHook(string $action): void
    {
        if (! method_exists($this, $action)) {
            ...
            throw new Exception(sprintf(__('Method %1$s does not exist in class %2$s', 'agentwp'), esc_html($action), esc_html(get_class($this))));
        }

        if ((new \ReflectionMethod($this, $action))->isPublic() === false) {
            // Translators: %1$s is the method name, %2$s is the class name.
            throw new Exception(sprintf(__('Method %1$s must be public in class %2$s', 'agentwp'), esc_html($action), esc_html(get_class($this))));
        }
    }
